...
-(void)hide{
    ...
    if ([self.delegate respondsToSelector:@selector(xhInputViewHideWithInputText:)]) {
        [self.delegate xhInputViewHideWithInputText:_textView.text];
    }
...
-(void)setupUI{
    ...
    switch (_style) {
        ...
        case InputViewStyleLarge:{
            ...
            gradientLayer.frame = _sendButton.bounds;
            gradientLayer.cornerRadius = 12;
            gradientLayer.startPoint = CGPointMake(0, 0.5);
            gradientLayer.endPoint = CGPointMake(1, 0.5);
            gradientLayer.locations = @[@(0),@(1.0)];//渐变点
            [gradientLayer setColors:@[(id)RGBA(232, 39, 37, 1).CGColor,(id)RGBA(241, 99, 34, 1).CGColor]];//渐变数组
            [_sendButton.layer addSublayer:gradientLayer];
